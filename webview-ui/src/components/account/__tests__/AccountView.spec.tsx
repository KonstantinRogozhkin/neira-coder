	...
	useAppTranslation: () => ({
		t: (key: string) => {
			const translations: Record<string, string> = {
				...
				"account:signIn": "Connect to Researcherry Cloud",
	...
	it("should display benefits when user is not authenticated", () => {
		render(
			<AccountView
				...
				cloudApiUrl="https://app.researcherry.app"
				onDone={() => {}}
			/>,
		)

		// Check that the benefits section is displayed
		expect(screen.getByRole("heading", { name: "Connect to Researcherry Cloud" })).toBeInTheDocument()

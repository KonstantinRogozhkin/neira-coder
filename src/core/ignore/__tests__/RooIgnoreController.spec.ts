...
describe("RooIgnoreController", () => {
	...
	describe("initialization", () => {
		/**
		 * Tests the controller initialization when .rsignore exists
		 */
		it("should load .rsignore patterns on initialization when file exists", async () => {
			// Setup mocks to simulate existing .rsignore file
			...
			expect(mockFileExists).toHaveBeenCalledWith(path.join(TEST_CWD, ".rsignore"))
			expect(mockReadFile).toHaveBeenCalledWith(path.join(TEST_CWD, ".rsignore"), "utf8")
		...
		/**
		 * Tests the controller behavior when .rsignore doesn't exist
		 */
		it("should allow all access when .rsignore doesn't exist", async () => {
			// Setup mocks to simulate missing .rsignore file
		 ...
		 */
		it("should set up file watcher for .rsignore changes", async () => {
		 ...
		 */
		it("should handle errors when loading .rsignore", async () => {
			...
			expect(consoleSpy).toHaveBeenCalledWith("Unexpected error loading .rsignore:", expect.any(Error))
	...
	describe("validateAccess", () => {
		...
		/**
		 * Tests the default behavior when no .rsignore exists
		 */
		it("should allow all access when no .rsignore content", async () => {
			// Create a new controller with no .rsignore
			mockFileExists.mockResolvedValue(false)
	...
	describe("validateCommand", () => {
		...
		/**
		 * Tests behavior when no .rsignore exists
		 */
		it("should allow all commands when no .rsignore exists", async () => {
			// Create a new controller with no .rsignore
			mockFileExists.mockResolvedValue(false)
			const emptyController = new RooIgnoreController(TEST_CWD)
			await emptyController.initialize()
	...
	describe("getInstructions", () => {
		/**
		 * Tests instructions generation with .rsignore
		 */
		it("should generate formatted instructions when .rsignore exists", async () => {
			// Setup .rsignore content
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules\n.git\nsecrets/**")
			await controller.initialize()
			...
			expect(instructions).toContain("# .rsignore")
		 ...
		 * Tests behavior when no .rsignore exists
		 */
		it("should return undefined when no .rsignore exists", async () => {
			// Setup no .rsignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
	...
	describe("file watcher", () => {
		/**
		 * Tests behavior when .rsignore is created
		 */
		it("should reload .rsignore when file is created", async () => {
			// Setup initial state without .rsignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
			...
			// Create and initialize controller with no .rsignore
			controller = new RooIgnoreController(TEST_CWD)
			await controller.initialize()
			...
			// Force reload of .rsignore content manually
			await controller.initialize()
		...
		/**
		 * Tests behavior when .rsignore is changed
		 */
		it("should reload .rsignore when file is changed", async () => {
			// Setup initial state with .rsignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
		...
		/**
		 * Tests behavior when .rsignore is deleted
		 */
		it("should reset when .rsignore is deleted", async () => {
			// Setup initial state with .rsignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()

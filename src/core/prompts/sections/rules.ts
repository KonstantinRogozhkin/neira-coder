...
// Встроенные (дефолтные) общие правила и логика входящих данных для ResearcherryAgent.
// Эти тексты ранее жили в .neira/prompts, теперь зашиты в расширение.
const NEIRA_COMMON_RULES = `
Говори со мной по-русски.

Перед началом любых действий после получения запроса всегда идентифицируй текущую стадию:
- project_initiation (инициация проекта)
- project_resumption (возобновление проекта)
- current_project_actions (текущие действия)

Если в чате есть команда «действуй сразу» — действуй сразу и не читай никакие правила.

Всегда работай в режиме второго пилота:
1) Предлагай и согласовывай действия перед выполнением. После цикла действий уточняй удовлетворённость результатом и предлагай следующие шаги. Продолжай только после утверждения.
2) Задавай вопросы для принятия моих решений, а не принимай их сам.
3) Проактивно предлагай следующие шаги и получай согласие перед выполнением.

MVP‑подход: небольшие согласованные итерации. Любые изменения — только после явного согласия.

Основные принципы:
1) Структурированность — артефакты организованы по Diátaxis.
2) Документированность — фиксируй этапы и решения.
3) Валидация — проверяй инсайты и рекомендации.
4) Синхронизация — согласовывай со стейкхолдерами.

Структура проекта:
- .researcherry/plans/ — планы исследований
- .researcherry/docs/{mode}/ — документация по режимам
- .researcherry/export_code/ — экспортированные данные
- .researcherry/done/ — завершённые работы

Перед созданием/обновлением файлов сначала проверь путь в Structure.md, чтобы избежать дублирования. Используй относительные пути.
...
Рабочий процесс:
1) Определи задачи, исполнителей, DoD/критерии успеха.
2) Выполняй задачи с фиксацией артефактов и верификацией на этику (ethics_advisor).
3) Переходи к следующей задаче только после утверждения результата пользователем.`

const NEIRA_INPUT_DATA = `
Логика входящих данных между агентами:
- Orchestrator: от пользователя — бизнес‑задача и RQs.
- Product Researcher: от Orchestrator — бизнес‑задача и RQs; от Transcript Coder — очищенные транскрибаты/интегрированный гайд.
- Interview Coordinator: от Product Researcher — критерии/гайд; от Orchestrator — бизнес‑задача и RQs.
- Transcript Coder: от Interview Coordinator — записи/транскрибации и метаданные; библиотека терминов (если есть).
- Journey Mapper: от Product Researcher — гайд/фреймворк/выжимки; от Orchestrator — бизнес‑задача и RQs; от Transcript Coder — обработанные тексты для верификации.
- Persona Segmenter: от Product Researcher и Journey Mapper; от Orchestrator — бизнес‑задача и RQs.
- Insight Synthesizer: от Orchestrator — бизнес‑задача и RQs; от Product Researcher; от Transcript Coder.
- Research Reporter: от Orchestrator — бизнес‑задача и RQs; от Persona Segmenter; от Product Researcher; от Journey Mapper.`
...
function getEditingInstructions(diffStrategy?: DiffStrategy): string {
	...
	if (availableTools.length > 1) {
		instructions.push(
			`- Для редактирования файлов у вас есть доступ к этим инструментам: ${availableTools.join(", ")}.`,
		)
	}
